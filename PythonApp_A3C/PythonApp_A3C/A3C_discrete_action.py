...
class ACNet(object):
        ...
        if scope == GLOBAL_NET_SCOPE:   # get global network
            ...
            with tf.compat.v1.variable_scope(scope):
                ...
                with tf.compat.v1.name_scope('a_loss'):
                    log_prob = tf.reduce_sum(input_tensor=tf.math.log(self.a_prob + 1e-5) * tf.one_hot(self.a_his, N_A, dtype=tf.float32), axis=1, keepdims=True)
                    exp_v = log_prob * tf.stop_gradient(td)
                    entropy = -tf.reduce_sum(input_tensor=self.a_prob * tf.math.log(self.a_prob + 1e-5),
    ...
    def _build_net(self, scope):
        ...
        with tf.compat.v1.variable_scope('actor'):
            ...
            a_prob = tf.compat.v1.layers.dense(l_a, N_A, tf.nn.softmax, kernel_initializer=w_init, name='ap')
